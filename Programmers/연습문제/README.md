...
행렬의 덧셈이라는 것 자체가 원래 노가다이다보니 구현된 코드도  
모든 원소에 대해서 반복문을 돌며 연산을 하는 방법 밖에 없다. (아닌가..?)

다 풀고 나서 조금 꺼림칙했던게 foreach나 map으로 작성하면 한 줄 코드나  
더 간단하게 구현할 수 있겠다는 생각이 들었지만 실천하지 않았다.
